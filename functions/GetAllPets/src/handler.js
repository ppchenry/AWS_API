...
const { validatePetRequest } = require("./middleware/guard");
const { logError } = require("./utils/logger");

/**
 * Routes that do not require a valid JWT.
 * NGO pet list is a public read endpoint (adoption website).
 */
const PUBLIC_RESOURCES = [
  "/pets/pet-list-ngo/{ngoId}",
];
...
async function handleRequest(event, context) {
  ...
  try {
    ...
    const validation = await validatePetRequest({ event });
